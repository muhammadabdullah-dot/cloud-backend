...
AGGREGATE_MODELS = (
    ...
    BranchReturn, BranchCreditCustomer,
)
...
async def apply_aggregates(branch: Branch, data: dict, source: str = "sync") -> dict:
    ...
    latest_day = max((r["day"] for r in daily if r.get("day")), default=None)
    ...
    async with in_transaction():
        ...
        await BranchTillClose.bulk_create([
            ...
            ) for r in (data.get("tillCloses") or [])
        ], batch_size=200)
        ...
        await BranchCreditCustomer.bulk_create([
            ...
            ) for r in (data.get("creditCustomers") or [])
        ], batch_size=200)
    ...
    return {
        ...
        "cashierDays": len(data.get("cashiers") or []),
...
async def add_stock_chunk(branch: Branch, snapshot_id: str, rows: list[dict]) -> int:
    ...
    await BranchProductStock.bulk_create([
        BranchProductStock(
            branch=branch, snapshot_id=snapshot_id,
            product_sku=r["sku"], product_name=r.get("name") or r["sku"],
            department=r.get("department"), category=r.get("category"), brand=r.get("brand"),
